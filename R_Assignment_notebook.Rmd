...
```
Percentage of homozygous/heterozygous/NA for maize and teosinte
```{r}
genotype_data<-as.data.frame(genotype_data)

homozygous_maize<-as.data.frame(matrix(ncol=1,nrow=nrow(genotype_data))) #adds homozygous column for maize
percent_NA_maize<-as.data.frame(matrix(ncol=1,nrow=nrow(genotype_data)))
heterozygous_maize<-as.data.frame(matrix(ncol=1,nrow=nrow(genotype_data)))

homozygous_teosinte<-as.data.frame(matrix(ncol=1,nrow=nrow(genotype_data))) #adds homozygous column for teosinte
percent_NA_teosinte<-as.data.frame(matrix(ncol=1,nrow=nrow(genotype_data)))
heterozygous_teosinte<-as.data.frame(matrix(ncol=1,nrow=nrow(genotype_data)))

homozygous_heterozygous_df_t_m<-cbind(genotype_data[c(1:3)],homozygous_maize,heterozygous_maize,percent_NA_maize, homozygous_teosinte, heterozygous_teosinte, percent_NA_teosinte, genotype_data[c(4:ncol(genotype_data))]) #makes the maize and teosinte homozygous, heterozygous, na percentile data frame


colnames(homozygous_heterozygous_df_t_m)[4]<-paste("Homozygous_percent_maize")
colnames(homozygous_heterozygous_df_t_m)[5]<-paste("Heterozygous_percent_maize")
colnames(homozygous_heterozygous_df_t_m)[6]<-paste("NA_percent_maize")

colnames(homozygous_heterozygous_df_t_m)[7]<-paste("Homozygous_percent_teosinte")
colnames(homozygous_heterozygous_df_t_m)[8]<-paste("Heterozygous_percent_teosinte")
colnames(homozygous_heterozygous_df_t_m)[9]<-paste("NA_percent_teosinte")
homozygous_heterozygous_df_t_m[homozygous_heterozygous_df_t_m=="?/?"] <- NA


for (i in 1:nrow(homozygous_heterozygous_df_t_m))
{
  SNPs<-homozygous_heterozygous_df_t_m[i,c(10:ncol(homozygous_heterozygous_df_t_m))]
  counter_maize<-0
  counter_teosinte<-0
  NA_teosinte<-0
  NA_maize<-0
  for(j in 1:ncol(SNPs)){
    if (is.na(SNPs[1,j])==TRUE){
      if ((homozygous_heterozygous_df_t_m[i,3]=="ZMMIL")|(homozygous_heterozygous_df_t_m[i,3]=="ZMMMR")|(homozygous_heterozygous_df_t_m[i,3]=="ZMMLR"))
      {
        NA_maize=NA_maize+1
      }
      if ((homozygous_heterozygous_df_t_m[i,3]=="ZMPIL")|(homozygous_heterozygous_df_t_m[i,3]=="ZMPBA")|(homozygous_heterozygous_df_t_m[i,3]=="ZMPJA"))
      {
          NA_teosinte=NA_teosinte+1
      }
      }
    if (is.na(SNPs[1,j])==FALSE){
  #if(homozygous_heterozygous_df_t_m[(homozygous_heterozygous_df_t_m$Group == "ZMMIL") | (homozygous_heterozygous_df_t_m$Group =="ZMMMR")| (homozygous_heterozygous_df_t_m$Group == "ZMMLR"), ] )
    
    if ((homozygous_heterozygous_df_t_m[i,3]=="ZMMIL")||(homozygous_heterozygous_df_t_m[i,3]=="ZMMMR")||(homozygous_heterozygous_df_t_m[i,3]=="ZMMLR"))
      {
   if((SNPs[1,j] == "A/A") | (SNPs[1,j] == "T/T")| (SNPs[1,j] == "C/C") | (SNPs[1,j] == "G/G")){
     counter_maize<-counter_maize +1;
      
    }
    }
    #if (homozygous_heterozygous_df_t_m[(homozygous_heterozygous_df_t_m$Group == "ZMPBA") | (homozygous_heterozygous_df_t_m$Group =="ZMPIL")| (homozygous_heterozygous_df_t_m$Group == "ZMPJA"), ] )
      if ((homozygous_heterozygous_df_t_m[i,3]=="ZMPIL")||(homozygous_heterozygous_df_t_m[i,3]=="ZMPBA")||(homozygous_heterozygous_df_t_m[i,3]=="ZMPJA"))
      {
      if((SNPs[1,j] == "A/A") | (SNPs[1,j] == "T/T")| (SNPs[1,j] == "C/C") | (SNPs[1,j] == "G/G")){
     counter_teosinte<-counter_teosinte +1;
  ...
  homozygous_heterozygous_df_t_m[i,6]<-NA_maize/ncol(SNPs) #NA_percent_maize
  homozygous_heterozygous_df_t_m[i,4]<-counter_maize/ncol(SNPs)#homozygous_maize
  homozygous_heterozygous_df_t_m[i,5]<-1-(homozygous_heterozygous_df_t_m[i,4])-(homozygous_heterozygous_df_t_m[i,6]) #heterozygous_maize
  
  homozygous_heterozygous_df_t_m[i,9]<-NA_teosinte/ncol(SNPs) #NA_percent_teosinte
  homozygous_heterozygous_df_t_m[i,7]<-counter_teosinte/ncol(SNPs)#homozygous_maize
  homozygous_heterozygous_df_t_m[i,8]<-1-(homozygous_heterozygous_df_t_m[i,7])-(homozygous_heterozygous_df_t_m[i,9]) #heterozygous_teosinte
  
  
}

temp_df<- melt(homozygous_heterozygous_df_t_m_sorted[c(1:9)], id= c("Sample_ID", "JG_OTU", "Group"))

ggplot(temp_df, aes(x=temp_df$Sample_ID, y=temp_df$value, fill=temp_df$variable))+geom_bar(stat="identity")+ xlab("Sample_Id") + ylab("Proportion")+ ggtitle("Proportionality of homozygous,heterozygous and NA SNPs in Maize and Teosinte")+ theme(legend.title=element_blank())
```
